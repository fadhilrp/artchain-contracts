...
app.post('/upload-ipfs', async (req, res) => {
  try {
    ...
    // For now, we'll assume IPFS uploads are more likely to be original
    ...
    const validated = await blockchain.validateArtwork(
      imageHash,
      isOriginal,
      isOriginal ? metadata.properties.artist : 'Unknown'
    );
    ...
    // 6. Save to database with IPFS data
    const artwork = await prisma.artwork.upsert({
      ...
      update: {
        ...
        originalAuthor: artworkDetails?.originalAuthor || metadata.properties?.artist || 'Unknown',
        updatedAt: new Date(),
      },
      create: {
        imageHash,
        ...
        originalAuthor: metadata.properties?.artist || 'Unknown',
        timestamp: new Date(),
      },
    });
